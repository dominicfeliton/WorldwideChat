...
public class WWCTranslate extends BasicCommand {
	...
	public boolean processCommand(boolean isGlobal) {
		...
		ActiveTranslator currTarget = main
		...
		if (args.length == 0 && !isGlobal) { /* User wants to see their own translation session */
			...
			return true;
		} else if (args.length == 1 && args[0] instanceof String && !isGlobal
				&& !args[0].equalsIgnoreCase(sender.getName())
				&& main.getServer().getPlayer(args[0]) != null) { /* User wants to see another translation session */
			if (sender.hasPermission("worldwidechat.wwct.otherplayers")) {
				WWCTranslateGUIMainMenu
						.getTranslateMainMenu(main.getServer().getPlayer(args[0]).getUniqueId().toString())
						.open((Player) sender);
			} else {
		...
		if (Bukkit.getServer().getPlayer(args[0]) == null && !isGlobal && currTarget instanceof ActiveTranslator) { // Don't
																													// let
																													// a
																													// person
																													// be
			...
			final TextComponent chatTranslationStopped = Component.text().append(main.getPluginPrefix().asComponent())
					.append(Component.text()
							.content(main.getConfigManager().getMessagesConfig()
									.getString("Messages.wwctTranslationStopped"))
							.color(NamedTextColor.LIGHT_PURPLE))
					.build();
		...
		} else if (args[0] instanceof String && Bukkit.getServer().getPlayer(args[0]) != null
				&& !Bukkit.getServer().getPlayer(args[0]).getName().equals(sender.getName()) && !isGlobal
				&& main.getActiveTranslator(
						Bukkit.getServer().getPlayer(args[0]).getUniqueId().toString()) instanceof ActiveTranslator) {
			...
			final TextComponent chatTranslationStoppedOtherPlayer = Component.text()
					...
					.append(Component.text()
							.content(main.getConfigManager().getMessagesConfig()
									.getString("Messages.wwctTranslationStoppedOtherPlayer").replace("%i", args[0]))
							.color(NamedTextColor.LIGHT_PURPLE))
					.build();
		...
		// If sender wants to overwrite /wwcg's existing translation session
			...
			final TextComponent chatTranslationStopped = Component.text().append(main.getPluginPrefix().asComponent())
					.append(Component.text()
							.content(main.getConfigManager().getMessagesConfig()
									.getString("Messages.wwcgTranslationStopped"))
							.color(NamedTextColor.LIGHT_PURPLE))
					.build();
		...
		if (args.length == 1 && args[0] instanceof String
				&& CommonDefinitions.getSupportedTranslatorLang(args[0]) != null) {
			// We got a valid lang code, continue and add player to ArrayList
			if (!isGlobal) {
				ActiveTranslator currTranslator = new ActiveTranslator(
						Bukkit.getServer().getPlayer(sender.getName()).getUniqueId().toString(), "None", args[0],
						false);
				main.addActiveTranslator(currTranslator);
				final TextComponent autoTranslate = Component.text().append(main.getPluginPrefix().asComponent())
						.append(Component.text()
								.content(main.getConfigManager().getMessagesConfig()
										.getString("Messages.wwctAutoTranslateStart").replace("%o", args[0]))
								.color(NamedTextColor.LIGHT_PURPLE))
						.build();
				adventureSender.sendMessage(autoTranslate);
			} else {
				ActiveTranslator currTranslator = new ActiveTranslator("GLOBAL-TRANSLATE-ENABLED", "None", args[0],
						false);
				main.addActiveTranslator(currTranslator);
				final TextComponent autoTranslate = Component.text().append(main.getPluginPrefix().asComponent())
						.append(Component.text()
								.content(main.getConfigManager().getMessagesConfig()
										.getString("Messages.wwcgAutoTranslateStart").replace("%o", args[0]))
								.color(NamedTextColor.LIGHT_PURPLE))
						.build();
				for (Player eaPlayer : Bukkit.getOnlinePlayers()) {
			...
			// Player has given us two arguments
				...
				&& CommonDefinitions.getSupportedTranslatorLang(args[0]) != null
				&& CommonDefinitions.getSupportedTranslatorLang(args[1]) != null) {
			...
			if (args[0].equalsIgnoreCase(args[1]) || (CommonDefinitions.isSameLang(args[0], args[1]))) {
				final TextComponent sameLangError = Component.text().append(main.getPluginPrefix().asComponent())
						.append(Component.text().content(
								main.getConfigManager().getMessagesConfig().getString("Messages.wwctSameLangError"))
								.color(NamedTextColor.RED))
						.build();
			...
			if (!isGlobal) {
				ActiveTranslator currTranslator = new ActiveTranslator(
						Bukkit.getServer().getPlayer(sender.getName()).getUniqueId().toString(), args[0], args[1],
						false);
				main.addActiveTranslator(currTranslator);
				final TextComponent langToLang = Component.text().append(main.getPluginPrefix().asComponent())
						.append(Component.text()
								.content(main.getConfigManager().getMessagesConfig()
										.getString("Messages.wwctLangToLangStart").replace("%i", args[0])
										.replace("%o", args[1]))
								.color(NamedTextColor.LIGHT_PURPLE))
						.build();
				adventureSender.sendMessage(langToLang);
			} else {
				ActiveTranslator currTranslator = new ActiveTranslator("GLOBAL-TRANSLATE-ENABLED", args[0], args[1],
						false);
				main.addActiveTranslator(currTranslator);
				final TextComponent langToLang = Component.text().append(main.getPluginPrefix().asComponent())
						.append(Component.text()
								.content(main.getConfigManager().getMessagesConfig()
										.getString("Messages.wwcgLangToLangStart").replace("%i", args[0])
										.replace("%o", args[1]))
								.color(NamedTextColor.LIGHT_PURPLE))
						.build();
			...
			return true;
			// Player has given us two arguments: Case 2 (First arg is another player who is
			// not ourselves...)
		} else if (args.length == 2 && args[0] instanceof String && args[1] instanceof String
			...
			if (sender.hasPermission("worldwidechat.wwct.otherplayers")) {
				if (Bukkit.getServer().getPlayer(args[0]) != null && (!(args[0].equals(sender.getName())))) {
					if (!isGlobal) {
						ActiveTranslator currTranslator = new ActiveTranslator(
								Bukkit.getServer().getPlayer(args[0]).getUniqueId().toString(), "None", args[1], false);
						main.addActiveTranslator(currTranslator);
						final TextComponent autoTranslateOtherPlayer = Component.text()
								.append(main.getPluginPrefix().asComponent())
								.append(Component.text()
										.content(main.getConfigManager().getMessagesConfig()
												.getString("Messages.wwctAutoTranslateStartOtherPlayer")
												.replace("%i", args[0]).replace("%o", args[1]))
										.color(NamedTextColor.LIGHT_PURPLE))
								.build();
						...
						final TextComponent autoTranslate = Component.text()
								...
								.append(Component.text()
										.content(main.getConfigManager().getMessagesConfig()
												.getString("Messages.wwctAutoTranslateStart").replace("%o", args[1]))
										.color(NamedTextColor.LIGHT_PURPLE))
								.build();
				...
				final TextComponent badPerms = Component.text().append(main.getPluginPrefix().asComponent())
						.append(Component.text()
								.content(main.getConfigManager().getMessagesConfig().getString("Messages.wwcBadPerms"))
								.color(NamedTextColor.RED))
						.append(Component.text().content(" (" + "worldwidechat.wwct.otherplayers" + ")")
			...
			// Player has given us three arguments
				...
				&& args[2] instanceof String && CommonDefinitions.getSupportedTranslatorLang(args[1]) != null
				&& CommonDefinitions.getSupportedTranslatorLang(args[2]) != null) {
			// If args[0] == a player, see if sender has perms to change player translation
			// preferences
			if (sender.hasPermission("worldwidechat.wwct.otherplayers")) {
				if (Bukkit.getServer().getPlayer(args[0]) != null && (!(args[0].equals(sender.getName())))) {
					// We got a valid lang code 2x, continue and add player to ArrayList
					if (args[1].equalsIgnoreCase(args[2]) || (CommonDefinitions.isSameLang(args[1], args[2]))) {
						final TextComponent sameLangError = Component.text()
								...
								.append(Component.text()
										.content(main.getConfigManager().getMessagesConfig()
												.getString("Messages.wwctSameLangError"))
										.color(NamedTextColor.RED))
								.build();
						adventureSender.sendMessage(sameLangError);
						return false;
					...
					ActiveTranslator currTranslator = new ActiveTranslator(
							Bukkit.getServer().getPlayer(args[0]).getUniqueId().toString(), args[1], args[2], false);
					main.addActiveTranslator(currTranslator);
					final TextComponent langToLangOtherPlayer = Component.text()
							.append(main.getPluginPrefix().asComponent())
							.append(Component.text()
									.content(main.getConfigManager().getMessagesConfig()
											.getString("Messages.wwctLangToLangStartOtherPlayer").replace("%o", args[0])
											.replace("%i", args[1]).replace("%e", args[2]))
									.color(NamedTextColor.LIGHT_PURPLE))
							.build();
					...
					final TextComponent langToLang = Component.text().append(main.getPluginPrefix().asComponent())
							.append(Component.text()
									.content(main.getConfigManager().getMessagesConfig()
											.getString("Messages.wwctLangToLangStart").replace("%i", args[1])
											.replace("%o", args[2]))
									.color(NamedTextColor.LIGHT_PURPLE))
							.build();
		...
		final TextComponent invalidLangCode = Component.text().append(main.getPluginPrefix().asComponent())
				.append(Component.text()
						.content(main.getConfigManager().getMessagesConfig().getString("Messages.wwctInvalidLangCode")
								.replace("%o", CommonDefinitions.getFormattedValidLangCodes()))
						.color(NamedTextColor.RED))
				.build();
