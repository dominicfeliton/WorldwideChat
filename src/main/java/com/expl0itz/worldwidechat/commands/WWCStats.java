        ...
        if (args.length == 0) {
        	...
        	if ((sender instanceof ConsoleCommandSender)) {
        		final TextComponent notATranslator = Component.text()
                        .append(main.getPluginPrefix().asComponent())
                        .append(Component.text().content(main.getConfigManager().getMessagesConfig().getString("Messages.wwcsNotATranslator").replace("%i", "console")))
                        .build();
            ...
            if (main.getPlayerRecord(((Player)sender).getUniqueId().toString(), false) != null) {
                ...
                if (main.getActiveTranslator(((Player)sender).getUniqueId().toString()) != null) { //is currently active
                    isActiveTranslator =  ChatColor.BOLD + "" + ChatColor.GREEN + "\u2713" ;
                }
                final TextComponent stats = Component.text()
                        .append(main.getPluginPrefix().asComponent())
                        .append(Component.text().content(main.getConfigManager().getMessagesConfig().getString("Messages.wwcsName").replace("%i", sender.getName())).color(NamedTextColor.GOLD).decoration(TextDecoration.BOLD, true))
                ...
                final TextComponent notATranslator = Component.text()
                    .append(main.getPluginPrefix().asComponent())
                    .append(Component.text().content(main.getConfigManager().getMessagesConfig().getString("Messages.wwcsNotATranslator").replace("%i", sender.getName())))
                    .build();
                adventureSender.sendMessage(notATranslator);
                return true;
            }
